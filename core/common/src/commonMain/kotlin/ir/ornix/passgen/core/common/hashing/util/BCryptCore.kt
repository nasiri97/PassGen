package ir.ornix.passgen.core.common.hashing.util

import ir.ornix.passgen.core.common.hashing.util.BCryptCore.generate


/**
 * A direct Kotlin port of Bouncy Castle's `org.bouncycastle.crypto.generators.BCrypt`
 * (core/src/main/java/org/bouncycastle/crypto/generators/BCrypt.java), which implements
 * the raw bcrypt function as designed by Niels Provos and David Mazières, corresponding
 * to the OpenBSD/C reference implementation.
 *
 * This exists so every KMP target — jvm, js, wasmJs, native — runs the exact same
 * byte-array-safe algorithm, rather than js/wasmJs delegating to a different library
 * (e.g. hash-wasm) with different password-termination semantics. Bouncy Castle's
 * `generate()` operates on `ByteArray` with explicit length, so an embedded 0x00 byte
 * in the middle of a password is treated as an ordinary data byte, never as a
 * C-string terminator that truncates the rest of the input.
 *
 * All constants below (the Blowfish P-array and S-boxes, seeded from the hex digits
 * of pi) and the algorithm structure are transcribed mechanically from Bouncy Castle's
 * source, then verified against BCryptTest.java's own published test vectors:
 *
 *   generate(passwordToByteArray("12341234"), hex("0102030405060708" + "0102030405060708"), 5)
 *     == hex("cdd19088721c50e5cb49a7b743d93b5a6e67bef0f700cd78")
 *   generate(passwordToByteArray("1234"), hex("0102030405060708" + "0102030405060708"), 5)
 *     == hex("02a3269aca2732484057b40c614204814cbfc2becd8e093e")
 */
@OptIn(ExperimentalUnsignedTypes::class)
internal object BCryptCore {

    private fun UIntArray.toSignedIntArray(): IntArray = IntArray(size) { this[it].toInt() }

    private val MAGIC_STRING: IntArray = uintArrayOf(
        0x4F727068u, 0x65616E42u, 0x65686F6Cu,
        0x64657253u, 0x63727944u, 0x6F756274u
    ).toSignedIntArray()

    private val KP: IntArray = uintArrayOf(
    ...
    private val KS0: IntArray = uintArrayOf(
    ...
    private val KS1: IntArray = uintArrayOf(
    ...
    private val KS2: IntArray = uintArrayOf(
    ...
    private val KS3: IntArray = uintArrayOf(
    ...
    private const val SBOX_SK = 256
    private const val P_SZ = ROUNDS + 2
    private const val MAX_PASSWORD_BYTES = 72

    /** One-shot, stateful working copy of the S-boxes/P-array for a single [generate] call. */
    private class State {
        val S = IntArray(SBOX_SK * 4)
        val P = IntArray(P_SZ)

        fun initState() {
            KS0.copyInto(S, 0)
            KS1.copyInto(S, SBOX_SK)
            KS2.copyInto(S, SBOX_SK * 2)
            KS3.copyInto(S, SBOX_SK * 3)
            KP.copyInto(P, 0)
        }

        fun f(x: Int): Int =
            ((S[x ushr 24 and 0xff] + S[SBOX_SK + (x ushr 16 and 0xff)]) xor
                    S[SBOX_SK * 2 + (x ushr 8 and 0xff)]) + S[SBOX_SK * 3 + (x and 0xff)]

        fun cyclicXorKey(key: ByteArray) {
        ...
        fun processTable(startXl: Int, startXr: Int, table: IntArray) {
            var xl = startXl
            var xr = startXr
            val size = table.size
            var s = 0
        ...
        fun processTableWithSalt(table: IntArray, salt32Bit: IntArray, iv1: Int, iv2: Int) {
            var xl = iv1 xor salt32Bit[0]
            var xr = iv2 xor salt32Bit[1]
            val size = table.size
            var s = 0
            while (s < size) {
                xl = xl xor P[0]
                ...
                table[s] = xr
                table[s + 1] = xl
                var yl = salt32Bit[2] xor xr
                var yr = salt32Bit[3] xor xl
                if (s + 2 >= size) break
                yl = yl xor P[0]
                i = 1
                while (i < ROUNDS) {
                    yr = yr xor f(yl) xor P[i]
                    yl = yl xor f(yr) xor P[i + 1]
                    i += 2
                }
                yr = yr xor P[ROUNDS + 1]
                table[s + 2] = yr
                table[s + 3] = yl
                xl = salt32Bit[0] xor yr
                xr = salt32Bit[1] xor yl
                s += 4
            }
        }

        fun encryptMagicString(): ByteArray {
            val text = MAGIC_STRING.copyOf()
            repeat(64) {
                var j = 0
                while (j < text.size) {
                    var left = text[j]
                    ...
                    var k = 1
                    while (k < ROUNDS) {
                        right = right xor f(left) xor P[k]
                        left = left xor f(right) xor P[k + 1]
                        k += 2
                    }
                    right = right xor P[ROUNDS + 1]
                    text[j] = right
                    text[j + 1] = left
                    j += 2
                }
        ...
        fun deriveRawKey(cost: Int, salt: ByteArray, psw: ByteArray): ByteArray {
            require(salt.size == 16) { "Invalid salt size: 16 bytes expected." }
            require(cost in 4..31) { "Illegal cost factor: 4 - 31 expected." }
            val effectivePsw = if (psw.isEmpty()) ByteArray(4) else psw

            initState()

            val salt32Bit = IntArray(4) { i ->
                ((salt[i * 4].toInt() and 0xff) shl 24) or
                        ((salt[i * 4 + 1].toInt() and 0xff) shl 16) or
                        ((salt[i * 4 + 2].toInt() and 0xff) shl 8) or
                        (salt[i * 4 + 3].toInt() and 0xff)
            }
            val salt32Bit2 = intArrayOf(salt32Bit[2], salt32Bit[3], salt32Bit[0], salt32Bit[1])

            cyclicXorKey(effectivePsw)
            processTableWithSalt(P, salt32Bit, 0, 0)
            processTableWithSalt(S, salt32Bit2, P[P.size - 2], P[P.size - 1])

            val rounds = 1 shl cost
    ...
    /**
     * Calculates the raw bcrypt hash, matching
     * `org.bouncycastle.crypto.generators.BCrypt.generate(byte[], byte[], int, boolean)`
     * exactly: [pwInput] is hashed by explicit length (never truncated at an embedded
     * 0x00 byte), and when [addTerminator] is true a single trailing 0x00 byte is
     * appended before hashing (unless [pwInput] is already at the 72-byte limit).
     *
     * @return the 24-byte (192 bit) raw bcrypt output.
     */
    fun generate(
        pwInput: ByteArray,
        salt: ByteArray,
        ...
        require(salt.size == 16) { "BCrypt salt must be 128 bits" }
        require(pwInput.size <= MAX_PASSWORD_BYTES) { "BCrypt password must be <= 72 bytes" }
        require(cost in 4..31) { "BCrypt cost must be from 4..31" }

        val effective = if (addTerminator && pwInput.size < MAX_PASSWORD_BYTES) {
            pwInput + byteArrayOf(0)
        } else {
            pwInput
        }

        return State().deriveRawKey(cost, salt, effective)
    }
}